...
export function MeasurementsSection({ measurements, onEdit, isEditing = false }: MeasurementsSectionProps) {
    ...
    const formatSizeRange = (range?: SizeRange): string => {
        ...
        if (max) return `até ${max} ${displayStandard}`;
        return '—';
    };
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            >
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900">Medidas & Tamanhos</h3>
                        <p className="text-sm text-gray-500">Seus tamanhos preferidos e medidas corporais</p>
                    </div>
                </div>
            ...
            {isExpanded && (
                <div className="px-4 pb-4 space-y-4">
                    ...
                    <div className="flex items-center space-x-2 pb-3 border-b border-gray-100">
                        <span className="text-sm text-gray-500 mr-2">Exibir em:</span>
                    ...
                    {!hasAnyData ? (
                        <div className="py-8 text-center">
                            <p className="text-gray-500">Nenhuma medida cadastrada ainda.</p>
                            {onEdit && (
                                <button
                                    onClick={onEdit}
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    {measurements?.height && (
                                        <div className="bg-gray-50 rounded-lg p-3">
                                            <p className="text-xs text-gray-500 uppercase tracking-wider">Altura</p>
                                            <p className="text-lg font-semibold text-gray-900">{measurements.height} cm</p>
                                        </div>
                                    )}
                                    {measurements?.weight && (
                                        <div className="bg-gray-50 rounded-lg p-3">
                                            <p className="text-xs text-gray-500 uppercase tracking-wider">Peso</p>
                                            <p className="text-lg font-semibold text-gray-900">{measurements.weight} kg</p>
                                        </div>
                                    )}
                                    {measurements?.chest && (
                                        <div className="bg-gray-50 rounded-lg p-3">
                                            <p className="text-xs text-gray-500 uppercase tracking-wider">Busto</p>
                                            <p className="text-lg font-semibold text-gray-900">{measurements.chest} cm</p>
                                        </div>
                                    )}
                                    {measurements?.waist && (
                                        <div className="bg-gray-50 rounded-lg p-3">
                                            <p className="text-xs text-gray-500 uppercase tracking-wider">Cintura</p>
                                            <p className="text-lg font-semibold text-gray-900">{measurements.waist} cm</p>
                                        </div>
                                    )}
                                    {measurements?.hips && (
                                        <div className="bg-gray-50 rounded-lg p-3">
                                            <p className="text-xs text-gray-500 uppercase tracking-wider">Quadril</p>
                                            <p className="text-lg font-semibold text-gray-900">{measurements.hips} cm</p>
                                        </div>
                                    )}
                                    {measurements?.inseam && (
                                        <div className="bg-gray-50 rounded-lg p-3">
                                            <p className="text-xs text-gray-500 uppercase tracking-wider">Entrepernas</p>
                                ...
                                <div className="space-y-3">
                                    ...
                                    {currentSizes.shoes && (currentSizes.shoes.min || currentSizes.shoes.max) && (
                                        <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                            <span className="text-gray-600">Calçados</span>
                                    ...
                                    {currentSizes.waist && (currentSizes.waist.min || currentSizes.waist.max) && (
                                        <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                            <span className="text-gray-600">Cintura (calças)</span>
                                    ...
                                    {currentSizes.tops && currentSizes.tops.length > 0 && (
                                        <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                            <span className="text-gray-600">Blusas/Camisetas</span>
                                    ...
                                    {currentSizes.bottoms && currentSizes.bottoms.length > 0 && (
                                        <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                            <span className="text-gray-600">Calças/Saias</span>
                                    ...
                                    {currentSizes.dresses && currentSizes.dresses.length > 0 && (
                                        <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                            <span className="text-gray-600">Vestidos</span>
                                    ...
                                    {currentSizes.rings && Object.keys(currentSizes.rings).length > 0 && (
                                        <div className="pt-2">
                                            <p className="text-sm font-medium text-gray-700 mb-2">Anéis</p>
