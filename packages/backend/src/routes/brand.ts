...
router.put(
  ...
  body('foundedDate').optional().isString().withMessage('Founded date must be a string'),
  body('foundedDatePrecision').optional().isIn(['year', 'month', 'day']).withMessage('Founded date precision must be year, month, or day'),
  body('foundedBy').optional().isLength({ max: 200 }).withMessage('Founded by must be 200 characters or less'),
