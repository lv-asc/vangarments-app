...
describe('AI Image Processing Integration', () => {
  ...
  describe('Complete Image Processing Workflow', () => {
    ...
    it('should process uploaded image through complete AI pipeline', async () => {
      ...
      expect(mockAWSService.removeBackground).toHaveBeenCalledWith(expect.any(Buffer));
      expect(mockAWSService.uploadImage).toHaveBeenCalledWith(
        Buffer.from('processed-image'),
        expect.stringMatching(/^processed\/\d+-nike-shirt\.jpg$/),
        'image/jpeg'
      );
      expect(mockAWSService.detectLabels).toHaveBeenCalledWith(Buffer.from('processed-image'));
      expect(mockAWSService.detectText).toHaveBeenCalledWith(Buffer.from('processed-image'));
      expect(mockAWSService.invokeFashionModel).toHaveBeenCalledWith(Buffer.from('processed-image'));
    ...
    it('should handle background removal failure gracefully', async () => {
      ...
      // Should still process with original image
      expect(mockAWSService.detectLabels).toHaveBeenCalledWith(expect.any(Buffer));
    ...
    it('should process image from URL through complete pipeline', async () => {
      ...
      expect(mockAWSService.removeBackground).toHaveBeenCalled();
      expect(mockAWSService.detectLabels).toHaveBeenCalled();
      expect(mockAWSService.detectText).toHaveBeenCalled();
      expect(mockAWSService.invokeFashionModel).toHaveBeenCalled();
  ...
  describe('Confidence Scoring Integration', () => {
    it('should calculate accurate confidence scores based on AI results', async () => {
      ...
      mockAWSService.detectText.mockResolvedValue([
        { Type: 'LINE', DetectedText: 'Adidas', Confidence: 96.1 }
      ]);
      mockAWSService.invokeFashionModel.mockResolvedValue({
        domain: 'APPAREL',
        brand: 'Adidas®',
        ...
        confidence: 0.94,
        brandConfidence: 96,
        pieceTypeConfidence: 93,
        colorConfidence: 91,
        materialConfidence: 88
      });
  ...
  describe('Background Removal and Enhancement', () => {
    it('should successfully remove background and enhance image', async () => {
      ...
      expect(mockAWSService.removeBackground).toHaveBeenCalledWith(originalBuffer);
      expect(mockAWSService.uploadImage).toHaveBeenCalledWith(
        processedBuffer,
        expect.any(String),
        'image/jpeg'
    ...
    it('should handle background removal service unavailability', async () => {
      ...
      // Should still detect labels using original image
      expect(mockAWSService.detectLabels).toHaveBeenCalledWith(expect.any(Buffer));
    });
  });
  ...
  describe('Error Recovery and Resilience', () => {
    ...
    it('should handle complete AI service failure gracefully', async () => {
      mockAWSService.removeBackground.mockRejectedValue(new Error('Service down'));
      mockAWSService.detectLabels.mockRejectedValue(new Error('Rekognition down'));
      mockAWSService.detectText.mockRejectedValue(new Error('Text detection down'));
      mockAWSService.invokeFashionModel.mockRejectedValue(new Error('Model down'));
