...
router.post('/', AuthUtils.authenticateToken, AuthUtils.requireRole(['admin']), async (req, res) => {
    try {
        const { name, rating, group } = req.body;
        ...
        const condition = await ConditionModel.create(name, rating, group);
        res.json(condition);
    } catch (error) {
...
router.put('/:id', AuthUtils.authenticateToken, AuthUtils.requireRole(['admin']), async (req, res) => {
    try {
        ...
        const condition = await ConditionModel.update(req.params.id, name, rating, group);
        if (!condition) {
