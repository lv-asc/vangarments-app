...
import { pageApi, PageTeamMember } from '@/lib/pageApi';
...
interface PageTeamManagementProps {
    pageId: string;
}
...
// Using same roles as Brands for now, can be customized
const PAGE_ROLES = ['CEO', 'CFO', 'Founder', 'CD', 'Marketing', 'Seller', 'Designer', 'Model', 'Ambassador', 'Other'];

export default function PageTeamManagement({ pageId }: PageTeamManagementProps) {
    const [members, setMembers] = useState<PageTeamMember[]>([]);
    const [loading, setLoading] = useState(true);
    const [showAddModal, setShowAddModal] = useState(false);
    const [showEditModal, setShowEditModal] = useState(false);
    const [selectedMember, setSelectedMember] = useState<PageTeamMember | null>(null);
    ...
    const loadTeam = async () => {
        try {
            setLoading(true);
            const team = await pageApi.getTeamMembers(pageId, false);
            setMembers(team);
        } catch (error) {
    ...
    const handleAddMember = async () => {
        ...
        try {
            await pageApi.addTeamMember(pageId, {
    ...
    const handleUpdateMember = async () => {
        ...
        try {
            await pageApi.updateTeamMember(pageId, selectedMember.id, {
                roles,
                title,
                isPublic
            });
    ...
    const handleRemoveMember = async (memberId: string) => {
        ...
        try {
            await pageApi.removeTeamMember(pageId, memberId);
    ...
    const openEditModal = (member: PageTeamMember) => {
        ...
        initialRoles = initialRoles.filter(r => PAGE_ROLES.includes(r));
