...
export default function UserDirectMessagePage() {
    ...
    const initConversation = async () => {
        try {
            setLoading(true);
            // Start or get direct conversation with username
            const result = await apiClient.startConversation([username]);
            setConversationId(result.id);

            // Load conversation details
            const conv = await apiClient.getConversation(result.id);
            setConversation(conv);
            apiClient.markConversationAsRead(result.id);

            // Load messages
            const messagesResult = await apiClient.getMessages(result.id);
            ...
            if (err.status === 401) {
                ...
                showAlert('User Not Found', 'The user you are trying to message could not be found.', 'danger');
            }
        } finally {
            setLoading(false);
        }
                        ...
                        {isRecording ? (
                            ...
                            <form onSubmit={handleSendMessage} className="flex items-end gap-2">
                                ...
                                <div className="flex-1 relative">
                                    {showMentions && conversation?.participants && (
                                        <MentionSuggestions
                                            query={mentionQuery}
                                    ...
                                    <textarea
                                        ...
                                        rows={1}
                                        className="w-full px-4 py-2.5 bg-gray-100 border-0 rounded-2xl resize-none focus:ring-2 focus:ring-gray-900 focus:bg-white transition-all text-sm"
                                ...
                                <button
                                    ...
                                    className="p-2.5 bg-gray-900 text-white rounded-full hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"
                                >
                                    {sendingMessage ? (
