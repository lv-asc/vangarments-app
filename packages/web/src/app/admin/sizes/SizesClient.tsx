...
import { TrashIcon, PencilSquareIcon, PlusIcon, FunnelIcon, Bars3Icon } from '@heroicons/react/24/outline';
...
function SortableSizeItem({ size, onEdit, onDelete, isSelected, onSelect }: { size: Size, onEdit: (s: Size) => void, onDelete: (id: string) => void, isSelected: boolean, onSelect: (e: React.MouseEvent) => void }) {
    const {
        attributes,
        listeners,
        setNodeRef,
    ...
    return (
        <li
            ref={setNodeRef}
            style={style}
            className={`block hover:bg-gray-50 border-b border-gray-200 last:border-0 ${isDragging ? 'bg-indigo-50 shadow-inner' : ''} ${isSelected ? 'bg-indigo-50' : ''}`}
        >
            <div className="px-4 py-4 flex items-center sm:px-6">
                ...
                <div
                    {...attributes}
                    {...listeners}
                    className="mr-4 cursor-grab active:cursor-grabbing text-gray-400 hover:text-indigo-600 p-1"
                ...
                <div className="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between">
                    ...
                    <div className="mt-4 flex-shrink-0 sm:mt-0 sm:ml-5 flex gap-2">
                        <button onClick={() => onEdit(size)} className="text-indigo-600 hover:text-indigo-900 bg-indigo-50 p-2 rounded-full">
                            <PencilSquareIcon className="h-5 w-5" />
                        </button>
                        <button onClick={() => onDelete(size.id)} className="text-red-600 hover:text-red-900 bg-red-50 p-2 rounded-full">
...
export default function AdminSizesPage() {
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            const oldIndex = sizes.findIndex((s) => s.id === activeId);
            const newIndex = sizes.findIndex((s) => s.id === overId);

            let newSizes: Size[];

            if (selectedIds.has(activeId) && selectedIds.size > 1) {
                // Multi-item reorder
                const selectedItems = sizes.filter(s => selectedIds.has(s.id));
                const remainingItems = sizes.filter(s => !selectedIds.has(s.id));

                // Find relative new index among remaining items
                // This is tricky: we want to place the block of selected items 
                // at the position where 'over' is.

                const overIndexInRemaining = remainingItems.findIndex(s => s.id === overId);

                // If overId is one of the selected items, we need a different approach
                // or just move the block to the closest non-selected neighbor.
                // But dnd-kit normally 'over' a non-dragging item.

                newSizes = [...remainingItems];
                const insertIndex = overIndexInRemaining !== -1 ? overIndexInRemaining : 0;

                // If moving down (new > old), insert after overId. If moving up, insert before.
                // Actually, simple splice usually works if we know the target.
                const finalInsertIndex = newIndex > oldIndex ? insertIndex + 1 : insertIndex;

                newSizes.splice(finalInsertIndex, 0, ...selectedItems);
            } else {
                // Single item reorder
            ...
            try {
                const orders = newSizes.map((size, index) => ({
                    id: size.id,
                    sortOrder: index + 1
                }));
                await apiClient.reorderSizes(orders);
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            {/* Multi-select Header */}
            {activeTab === 'sizes' && selectedIds.size > 0 && (
                <div className="flex items-center justify-between mb-4 bg-indigo-50 p-3 rounded-lg border border-indigo-100 shadow-sm">
                    <div className="flex items-center space-x-3">
                        <input
                            type="checkbox"
                            checked={sizes.length > 0 && selectedIds.size === sizes.length}
                            onChange={handleSelectAll}
                            className="h-5 w-5 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                        />
                        <span className="text-sm font-semibold text-indigo-900">{selectedIds.size} sizes selected</span>
                        <span className="text-xs text-indigo-700 italic border-l border-indigo-200 pl-3 ml-3">Drag any selected item to move the whole group</span>
                    </div>
                    <button
                        onClick={clearSelection}
                        className="text-xs bg-white text-indigo-600 px-3 py-1.5 rounded-md border border-indigo-200 hover:bg-indigo-100 transition-colors font-medium shadow-sm"
                    >
                        Clear Selection
                    </button>
                </div>
                            ...
                            <ul role="list" className="divide-y divide-gray-200">
                                {sizes.map((size) => (
                                    <SortableSizeItem
                                        ...
                                        isSelected={isSelected(size.id)}
                                        onSelect={(e) => handleItemClick(size.id, e)}
                                    />
                                ))}
                            </ul>
                        </SortableContext>
