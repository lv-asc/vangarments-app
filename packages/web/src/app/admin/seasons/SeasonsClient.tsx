...
import { TrashIcon, PencilSquareIcon, PlusIcon, SunIcon } from '@heroicons/react/24/outline';
...
export default function AdminSeasonsPage() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            // Try to get seasons, fallback to empty if endpoint doesn't exist
            const data = await apiClient.getVUFSAttributeValues('season').catch(() => []);
            setSeasons(Array.isArray(data) ? data : (data?.values || []));
        } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="mt-2 text-sm text-gray-700">Manage seasonal collections.</p>
                </div>
                <button
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-md">
                <ul role="list" className="divide-y divide-gray-200">
                    {seasons.map((season) => (
                        <li key={season.id} className="block hover:bg-gray-50">
                            <div className="px-4 py-4 flex items-center sm:px-6">
                                <div className="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between">
                                    <div className="flex items-center">
                                        <SunIcon className="h-8 w-8 text-amber-500 mr-4" />
                                        <p className="font-medium text-amber-600">{season.name}</p>
                                    </div>
                    ...
                    {seasons.length === 0 && (
                        <li className="px-4 py-8 text-center text-gray-500">
                            No seasons found. Add a season to get started.
                        </li>
                    )}
                </ul>
            </div>
                    ...
                    <div className="bg-white rounded-lg p-6 max-w-md w-full">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. Spring/Summer 2024, Fall/Winter 2023"
                            />
                        </div>
