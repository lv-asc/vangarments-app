...
export default function AdminEditStorePage() {
    ...
    const [activeTab, setActiveTab] = useState<'details' | 'skus' | 'drops' | 'lookbooks' | 'team'>('details');
    ...
    const loadStore = async () => {
        try {
            setLoading(true);
            const loadedStore = await brandApi.getBrand(storeId);
            if (!loadedStore) {
            ...
            setFormData({
                storeName: loadedStore.brandInfo.name || '',
                slug: loadedStore.brandInfo.slug || '',
                description: loadedStore.brandInfo.description || '',
                website: loadedStore.brandInfo.website || '',
            ...
            // Initialize Banners
            const mainBanner = loadedStore.brandInfo.banner;
            const additionalBanners = loadedStore.brandInfo.banners || [];
            ...
            const mainLogoUrl = loadedStore.brandInfo.logo;
            const additionalLogos = loadedStore.profileData?.additionalLogos || [];
            const logoMetadata = loadedStore.profileData?.logoMetadata || [];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const existingContactInfo = store?.brandInfo?.contactInfo || {};
            ...
            const storeSlug = formData.slug ? slugify(formData.slug) : slugify(formData.storeName);

            // 1. Update Store Info
            await brandApi.updateBrand(targetStoreId, {
                brandInfo: {
            ...
            if (storeSlug) {
                window.history.replaceState(null, '', `/admin/stores/${storeSlug}`);
            }
    ...
    const handleDelete = async () => {
        try {
            setDeleting(true);
            await brandApi.deleteBrand(store?.id || storeId);
    ...
    const displaySlug = store?.brandInfo?.slug || slugify(store?.brandInfo?.name || '') || storeId;
            ...
            >
                <div>
                    <p className="text-sm text-gray-500 mb-4">
                        Are you sure you want to delete this store? It will be moved to trash and can be restored later.
                    </p>
            ...
            <div className="bg-white shadow rounded-lg overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">Update store details, drops, lookbooks and official SKUs.</p>
                    </div>
                </div>
                ...
                {activeTab === 'details' ? (
                            ...
                            <div className="col-span-2">
                                ...
                                <div className="mt-1">
                                    ...
                                    <p className="mt-1 text-xs text-gray-500">
                                        Unique identifier for this store. Useful for distinguishing duplicate names.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="col-span-2">
                                ...
                                <div className="mt-1 flex rounded-md shadow-sm">
                                    ...
                                    <input
                                        ...
                                        value={formData.slug}
                                        onChange={handleChange}
                                        placeholder={slugify(formData.storeName)}
                                ...
                                <p className="mt-1 text-xs text-gray-500">
                                    The URL-friendly version of the name. e.g. <code>my-store-name</code>. Leave empty to auto-generate.
                                </p>
                            </div>
                    ...
                    <div className="p-6">
                        <CollectionManagement brandId={store?.id || storeId} title="Drops" itemName="Drop" hideSeason={true} />
                    </div>
                ) : activeTab === 'lookbooks' ? (
                    <div className="p-6">
                        <LookbookManagement brandId={store?.id || storeId} />
                    </div>
                ) : activeTab === 'team' ? (
                    <div className="p-6">
                        <TeamManagement brandId={store?.id || storeId} />
                    </div>
                ) : (
                    <div className="p-6">
                        <SKUManagement brandId={store?.id || storeId} />
                    </div>
                )}
            </div>
        </div >
