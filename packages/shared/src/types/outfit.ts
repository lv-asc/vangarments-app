// Outfit creation and styling types

export interface OutfitItem {
  vufsItemId: string;
  position: OutfitPosition;
  layerOrder: number; // For layering clothes (0 = base layer, higher = outer layers)
}
...
export interface OutfitSuggestion {
  id: string;
  baseItemId: string; // The item user pinned/selected
  suggestedItems: {
    itemId: string;
    position: OutfitPosition;
    confidence: number;
...
export interface OutfitFilters {
  ...
  style?: string[];
  colors?: string[];
  hasItem?: string; // Filter by specific item ID
  isPublic?: boolean;
  isFavorite?: boolean;
  userId?: string;
  search?: string;
...
export interface SizeRecommendation {
  ...
  userMeasurements: {
    height: number;
    weight: number;
    sizes: Record<string, string>; // Size standards
  };
  itemSizing: {
    brand: string;
...
// Outfit creation workflow states
export type OutfitCreationStep = 
  | 'select_base_item'
  | 'add_complementary_items'
  | 'adjust_styling'
  | 'review_outfit'
  | 'save_outfit';
...
export interface OutfitCreationSession {
  ...
  pinnedItem?: string; // Base item user selected
