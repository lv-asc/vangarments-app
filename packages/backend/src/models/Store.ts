...
export class StoreModel {
    ...
    static async create(data: { name: string; slug?: string; description?: string; location?: string; userId?: string; email?: string; telephone?: string; socialLinks?: Array<{ platform: string; url: string }> }): Promise<Store> {
        // Auto-generate slug from name if not provided
        const slug = data.slug || slugify(data.name);

        const query = `
      INSERT INTO stores (name, slug, description, location, user_id, email, telephone, social_links)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING *
    `;
        const result = await db.query(query, [data.name, slug, data.description || null, data.location || null, data.userId || null, data.email || null, data.telephone || null, JSON.stringify(data.socialLinks || [])]);
        return this.mapRowToStore(result.rows[0]);
    }

    static async update(id: string, data: { name?: string; description?: string; location?: string; userId?: string | null; email?: string; telephone?: string; socialLinks?: Array<{ platform: string; url: string }> }): Promise<Store | null> {
