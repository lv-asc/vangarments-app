...
import { HomiesModals } from './HomiesModals';
...
export default function ProfilePage() {
  ...
  const [showAddHomieModal, setShowAddHomieModal] = useState(false); // Added state
  ...
  // Homies state
  const [myHomiesLists, setMyHomiesLists] = useState<any[]>([]);
  const [isHomiesModalOpen, setIsHomiesModalOpen] = useState(false);
  const [editingHomiesList, setEditingHomiesList] = useState<any>(null);
  const [homiesListMembers, setHomiesListMembers] = useState<any[]>([]);
  const [loadingHomiesMembers, setLoadingHomiesMembers] = useState(false);
  ...
  // Fetch feed/motion content when tab is active
  useEffect(() => {
    if (user?.id && (activeTab === 'feed' || activeTab === 'motion')) {
      fetchFeed(activeTab === 'motion' ? 'motion' : 'feed', true, user.id);
    }
  ...
  useEffect(() => {
    const fetchListings = async () => {
      ...
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/marketplace/seller/${userProfile.id}`);
    ...
    if (activeTab === 'listings' && userProfile?.id) {
      fetchListings();
    }
  ...
  const loadMyHomiesLists = async () => {
    try {
      const lists = await apiClient.getMyHomiesLists();
      setMyHomiesLists(lists || []);
    } catch (err) {
      console.error('Failed to load my homies lists:', err);
    }
  };
  ...
  const handleOpenHomiesManagement = (list: any = null) => {
    setEditingHomiesList(list);
    setHomiesListMembers([]);
    setIsHomiesModalOpen(true);
  ...
  const loadListMembers = async (listId: string) => {
    setLoadingHomiesMembers(true);
    try {
      const members = await apiClient.getHomiesListMembers(listId);
      setHomiesListMembers(members || []);
    } catch (err) {
      console.error('Failed to load list members:', err);
    } finally {
      setLoadingHomiesMembers(false);
    }
  };

  const handleCreateOrUpdateHomiesList = async (name: string, color: string) => {
    try {
      if (editingHomiesList) {
        await apiClient.updateHomiesList(editingHomiesList.id, { name, color });
        toast.success('List updated!');
      } else {
        await apiClient.createHomiesList(name, color);
        toast.success('List created!');
      }
      loadMyHomiesLists();
      setIsHomiesModalOpen(false);
      setEditingHomiesList(null);
    } catch (err: any) {
      toast.error('Failed to save list: ' + err.message);
    }
  };

  const handleDeleteHomiesList = async (listId: string) => {
    if (!confirm('Are you sure you want to delete this list?')) return;
    try {
      await apiClient.deleteHomiesList(listId);
  ...
  // Trigger instant search when Add Member modal opens
  useEffect(() => {
  ...
  const handleMemberSearch = async (query: string) => {
    ...
    // Allow empty query for "instant search" suggestions

    setSearchingMembers(true);
    try {
      const results = await apiClient.searchHomiesCandidates(query);
  ...
  const handleAddMemberToList = async (member: any) => {
    if (!editingHomiesList) return;
    try {
      // Pass member.id and member.type (defaulting to 'user' if undefined)
      await apiClient.addHomiesMember(editingHomiesList.id, member.id, member.type || 'user');
      toast.success('Member added!');
      loadListMembers(editingHomiesList.id);
      loadMyHomiesLists(); // To update member count
  ...
  const handleRemoveMemberFromList = async (memberId: string) => {
    if (!editingHomiesList) return;
    try {
      await apiClient.removeMemberFromHomiesList(editingHomiesList.id, memberId);
  ...
  const handleSaveMeasurements = async (newMeasurements: { height?: number; weight?: number }) => {
    try {
      await apiClient.updateProfile({
      ...
      toast.error('Failed to update measurements: ' + (err.message || 'Unknown error'));
    }
  };

  const handleHomieAdded = () => {
    // This function might be called when a homie is added,
    // potentially to refresh lists or show a toast.
    // For now, it can just log or show a generic success.
    toast.success('Homie added successfully!');
    // You might want to reload homies lists here if needed
    loadMyHomiesLists();
  };
    ...
    <>
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {/* Homies Management Section */}
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                <HomiesIcon className="w-5 h-5 text-indigo-600" />
                Manage Homies Lists
              </h3>
              <button
                onClick={() => handleOpenHomiesManagement()}
            ...
            {myHomiesLists.length === 0 ? (
              <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                <p className="text-gray-500 text-sm">Create lists to group your friends.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {myHomiesLists.map(list => (
                  <div
                    key={list.id}
                    className="flex items-center justify-between p-4 rounded-xl border border-gray-100 bg-white hover:border-indigo-100 hover:shadow-sm transition-all cursor-pointer group"
                    onClick={() => handleOpenHomiesManagement(list)}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">{list.name}</h4>
                          {list.isDefault && (
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-gray-100 text-gray-500 border border-gray-200 uppercase tracking-wider">
                              Standard
                            </span>
                          )}
                        </div>
                        <p className="text-xs text-gray-500">{list.memberCount || 0} members</p>
                      </div>
                    </div>
                    <ChevronRightIcon className="w-4 h-4 text-gray-400 group-hover:text-indigo-400" />
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="p-6">
              ...
              {activeTab === 'feed' && (
                <FeedGallery
                  posts={feedPosts.filter(p => p.userId === user?.id && p.contentType === 'feed')}
                />
              )}
              ...
              {activeTab === 'motion' && (
                <div className="h-[600px] rounded-xl overflow-hidden bg-black">
                  <MotionFeed
                    posts={feedPosts.filter(p => p.userId === user?.id && p.contentType === 'motion')}
                  />
                </div>
              )}
                ...
                <>
                  ...
                  {loadingListings ? (
                    ...
                    <div className="text-center py-12 border-2 border-dashed border-gray-100 rounded-xl">
                      ...
                      <p className="text-gray-500">You have no items for sale.</p>
                    </div>
                  )}
                </>
              )}

              {activeTab !== 'wardrobe' && activeTab !== 'likes' && activeTab !== 'wishlists' && activeTab !== 'feed' && activeTab !== 'motion' && activeTab !== 'listings' && (
      ...
      {/* Homies Modals */}
      <HomiesModals
        isHomiesModalOpen={isHomiesModalOpen}
        setIsHomiesModalOpen={setIsHomiesModalOpen}
        editingHomiesList={editingHomiesList}
        handleCreateOrUpdateHomiesList={handleCreateOrUpdateHomiesList}
        handleDeleteHomiesList={handleDeleteHomiesList}
        homiesListMembers={homiesListMembers}
        loadingHomiesMembers={loadingHomiesMembers}
        setShowAddMemberModal={setShowAddMemberModal}
        handleRemoveMemberFromList={handleRemoveMemberFromList}
        showAddMemberModal={showAddMemberModal}
        memberSearchQuery={memberSearchQuery}
        handleMemberSearch={handleMemberSearch}
        memberSearchResults={memberSearchResults}
        searchingMembers={searchingMembers}
        handleAddMemberToList={handleAddMemberToList}
      />
    </>
  );
};
