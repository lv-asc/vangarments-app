...
import CommentThread from '@/components/design-studio/CommentThread';
import type { InfiniteCanvasRef, CanvasObject, CanvasTool, CanvasComment } from '@/components/design-studio/InfiniteCanvas';
...
export default function MoodboardEditorPage() {
    ...
    const [activeCommentThread, setActiveCommentThread] = useState<{ id: string; comments: CanvasComment[]; position: { x: number; y: number } } | null>(null);
    ...
    const handleSave = async () => {
        if (!moodboard || !canvasRef.current || !isAuthenticated) return;
            ...
            <div className="flex-1 relative overflow-hidden">
                    ...
                    <InfiniteCanvas
                        ...
                        onObjectSelected={(obj, pos) => {
                            ...
                            if ((obj as any).isCommentMarker && pos) {
                                ...
                                setSelectedObject(null); // Don't show properties toolbar for comments
                            } else {
                                setActiveCommentThread(null);
                ...
                {activeCommentThread && (
                    <CommentThread
                        ...
                        position={activeCommentThread.position}
                        currentUser={{
                            id: user?.id || 'guest',
                        ...
                        onAddReply={(text) => {
                            if (!canvasRef.current || !activeCommentThread) return;

                            const newComment: CanvasComment = {
                                id: `c_${Date.now()}`,
                                userId: user?.id || 'guest',
                                userName: user?.profile?.name || 'Guest User',
                                userAvatar: user?.profile?.profilePicture,
                                content: text,
                                createdAt: new Date().toISOString()
                            };

                            // Update local state
                            const updatedComments = [...activeCommentThread.comments, newComment];
                            setActiveCommentThread({
                                ...activeCommentThread,
                            ...
                            const obj = canvasRef.current.getObjectById(activeCommentThread.id);
                            if (obj) {
                        ...
                        onResolve={() => {
                            if (canvasRef.current && activeCommentThread) {
                                canvasRef.current.deleteObjectById(activeCommentThread.id);
                                setActiveCommentThread(null);
                                setSaved(false);
                            }
                        }}
