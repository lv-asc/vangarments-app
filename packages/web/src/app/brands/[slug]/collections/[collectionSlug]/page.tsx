                    ...
                    {items && items.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {items.map((item: any, idx: number) => (
                                <div key={item.itemId || idx} className="group">
                                    <div className="aspect-[3/4] bg-gray-100 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                        {item.item?.images?.[0] || item.catalogItem?.images?.[0] ? (
                                            <img
                                                src={getImageUrl(item.item?.images?.[0] || item.catalogItem?.images?.[0])}
                                                alt={item.item?.name || item.catalogItem?.name || 'Item'}
                                                className="w-full h-full object-cover"
                                            />
                                        ) : (
                                    ...
                                    <div className="mt-3">
                                        <h3 className="font-medium text-gray-900 group-hover:text-blue-600 transition-colors">
                                            {item.item?.name || item.catalogItem?.name || `Item ${idx + 1}`}
                                        </h3>
                                        {(item.item?.description || item.catalogItem?.description) && (
